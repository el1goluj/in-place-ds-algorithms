...
// GPU version
double gpu_select(cl_device_id device, cl_context clContext, T* matrix, int numElements, int rem){
  ...
  cl_kernel clKernel;
  if(!rem){
    clKernel = clCreateKernel(clProgram,"select_copy_if", &clStatus);
    CL_ERR();
  }
  else{
    clKernel = clCreateKernel(clProgram,"select_remove_if", &clStatus);
    CL_ERR();
  }

  // Allocate matrix
  cl_mem d_matrix = clCreateBuffer(clContext, CL_MEM_READ_WRITE, numElements * sizeof(T), NULL, &clStatus);
  cl_mem d_matrix_out = clCreateBuffer(clContext, CL_MEM_READ_WRITE, numElements * sizeof(T), NULL, &clStatus);
  ...
  if(!rem)
    clSetKernelArg(clKernel, 0, sizeof(cl_mem), &d_matrix_out);
  else
  ...
  if(!rem)
    clStatus = clEnqueueReadBuffer(clCommandQueue, d_matrix_out, CL_TRUE, 0, numElements * sizeof(T), matrix, 0, NULL, NULL);
  else
  ...
  clReleaseMemObject(d_matrix_out);
...
int bench(cl_device_id device, cl_context clContext, int numElements, int input, int rem) {
  T* M = (T*) malloc (numElements * sizeof(T));
  T* M1 = (T*) malloc (numElements * sizeof(T));
  T* M2 = (T*) malloc (numElements * sizeof(T));
  T* M3 = (T*) malloc (numElements * sizeof(T));
  ...
  // CPU execution
  if(!rem)
    cpu_copy_if(M3, M2, numElements, is_even());
  else
    cpu_remove_if(M2, numElements, is_even());
  ...
  // GPU execution
  double bw = gpu_select(device, clContext, M, numElements, rem);
...
int main(const int argc, const char* argv[]) {
  ...
  if (argc != 6) {
      printf("Wrong format\n");
      printf("Syntax: %s <Platform Device Input (%% elements) numElements remove(1)_or_copy(0)>\n",argv[0]);
      exit(1);
  }
  ...
  bench(clDevices[device], clContext, numElements, input, rem);
