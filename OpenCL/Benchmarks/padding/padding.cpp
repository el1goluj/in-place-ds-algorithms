...
// GPU version
double gpu_padding(cl_device_id device, cl_context clContext, T* matrix, int x_size, int y_size, int pad_size){
  ...
  // Create kernel
  cl_kernel clKernel = clCreateKernel(clProgram,"padding", &clStatus);
  CL_ERR();
...
int bench(cl_device_id device, cl_context clContext, int m, int n, int pad) {
  ...
  // GPU execution
  double bw = gpu_padding(device, clContext, M, n, m, n+pad);
