...
// GPU version
double gpu_unique(cl_device_id device, cl_context clContext, T* matrix, int numElements){
  ...
  // Create kernel
  cl_kernel clKernel = clCreateKernel(clProgram,"unique", &clStatus);
  CL_ERR();
...
int bench(cl_device_id device, cl_context clContext, int numElements, int input) {
  ...
  for(int i = 0; i < numElements; i++){
    M[i] = value1;
    if(i >= numElements/4 && i < numElements/2) M[i] = value2;
    if(i >= numElements/2 && i < 3*numElements/4) M[i] = value3;
    if(i >= 3*numElements/4 && i < numElements) M[i] = value4;
  }
  int Ma = (numElements * input)/100;
  int ma = Ma;
  while(ma>0){
    int x = (int)(numElements*(((float)rand()/(float)RAND_MAX)));
    if(M[x]==value1 || M[x]==value2 || M[x]==value3 || M[x]==value4){
      M[x] = x+2;
      ma--;
    }
  }
  ...
  // GPU execution
  double bw = gpu_unique(device, clContext, M, numElements);
