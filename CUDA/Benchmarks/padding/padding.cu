...
double cpu_padding(T* matrix, int x_size, int y_size, int pad_size){
...
// GPU version
double gpu_padding(T* matrix, int x_size, int y_size, int pad_size){
  ...
  // Kernel launch 
  padding<<<num_wg, ldim>>>(d_matrix, x_size, pad_size, y_size, d_flags);
  ...
  // Compute bandwidth
  double bandwidth = 2*x_size * y_size * sizeof(T) / (time_kernel * 1000000.0);
...
int bench(int m, int n, int pad) {
  ...
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < n; j++) {
      M[i * n + j] = (T)rand() / RAND_MAX;
      M2[i * n + j] = M[i * n + j];
    }
  }
  for (int i = n * m; i < (n + pad) * m; i++){
    M[i] = 0.0f;
    M2[i] = 0.0f;
  }
...
#if TEST_CPU
  // CPU execution
  double bw_cpu = cpu_padding(M2, n, m, n+pad);
  ...
  // GPU execution
  double bw = gpu_padding(M, n, m, n+pad);
  ...
  for (int i = 0; i < m ; i++) {
    for (int j = 0; j < n+pad; j++) {
      if( N[i * (n+pad) + j] != M[i * (n+pad) + j] ) {
        std::cerr << "Error. i = " << i << "; j = " << j << " " << N[i * (n+pad) + j] << " " <<  M[i * (n+pad) + j] <<std::endl;
        return 1;
      }
#if TEST_CPU
      if( N[i * (n+pad) + j] != M2[i * (n+pad) + j] ) {
        std::cerr << "Error-CPU. i = " << i << "; j = " << j << "\t" << N[i * (n+pad) + j] << "\t" << M2[i * (n+pad) + j] <<std::endl;
        return 1;
      }
#endif
    }
